...
**Auditoría SEO + investigación de keywords 2026 (14 jul 2026, commit `1d66168`):** informe completo entregado como documento aparte (`SEO_Infinexa_Julio2026.docx`, fuera del repo — mismo criterio que el informe de la sección 1.8). Correcciones técnicas aplicadas: `<meta name="description">` agregada a `index.html`, `servicios/index.html` e `infografia/index.html` (antes solo tenían `og:description`, que no alimenta el resumen de Google); título de `/servicios/` reescrito con intención de búsqueda real; `BreadcrumbList` JSON-LD del blog corregido (apuntaba a `/blog/` en vez de `/aprender/`, pendiente arrastrado desde la sección 1.14). Auditoría de solo lectura de cobertura de keywords en los 16 posts: cobertura semántica buena, sin necesidad de forzar coincidencia exacta (se descartó explícitamente el keyword stuffing por ir contra la evidencia de 2026). Hallazgos clave de la investigación: AI Overviews de Google aparecen en ~26% de búsquedas en EE.UU. y bajan 59% el CTR del top orgánico cuando aparecen — el contenido narrativo/original del blog (arco Bitcoin, callback a Carlos) está mejor posicionado frente a esto que contenido genérico; contenido financiero pesa ~3x más en E-E-A-T (YMYL) que el promedio, Infinexa ya tiene la base (autor con credenciales, `/nosotros/`, `/transparencia/`); FAQ/HowTo rich results fueron eliminados de Google en 2026, sin ganancia visual de agregarlos. **Pendiente identificado, no ejecutado:** post de remesas y USDT (datos frescos ya investigados — comisiones <1% con USDT/TRC-20 vs. hasta 10% en remesadoras tradicionales, contexto de nuevo impuesto de EE.UU. a remesas), próximo candidato de mayor oportunidad.
...
0c. ~~Retrofit de las páginas preexistentes a los includes compartidos~~ — ✅ **Resuelto el 13 jul 2026** (commit `0650ff8`), en la parte que quedaba pendiente y era segura de migrar. El nav ya se había resuelto en Fase 1.5B/1.5D. En esta sesión se migraron las últimas 4 páginas (`/`, `/servicios/`, `/infografia/`, `/diversifica/`) de su código en línea duplicado a los includes compartidos `_includes/ga4.html`, `_includes/analytics.html`, `_includes/qr-share.html` y `_includes/reduced-motion.html` — mismos que ya usaban `/diagnostico/`, `/talleres/`, `/nosotros/`, `/contacto/`, `/aprender/` y el layout del blog. En cada archivo se conservó, en su propio `<script>` separado, la lógica específica de esa página (auto-wiring de CTAs y formularios en home, FAQ/lead-form en servicios, disparo de `innovation_guide_opened` en infografia, wiring del CTA en diversifica) — solo se extrajo la función `trackEvent()` en sí, no el resto del script. **Verificado en vivo** (Claude in Chrome) en las 4 páginas: `gtag`/`trackEvent` definidos correctamente, QR renderizando una sola vez (sin duplicados), logo único en infografia, y la clave de Web3Forms de servicios intacta. **Se conserva intencionalmente sin migrar:** el footer con firma embebida en estas 4 páginas (no usa `_includes/footer.html`, para no duplicar la firma — mismo criterio ya aplicado a Home en fases anteriores).
