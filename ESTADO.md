...
0. **🔴 Nuevo prioritario (Fase 1.5, 13 jul 2026) — Proporcionar la clave real de Web3Forms.** Crear cuenta en web3forms.com, confirmar el correo, y compartir la clave de acceso para reemplazar el placeholder `WEB3FORMS_ACCESS_KEY_PENDIENTE` (usado en `/diagnostico/`, `/talleres/`, `/contacto/`, `/servicios/`). Sin esto, ningún formulario del sitio entrega correo real.
0b. **Pruebas responsive visuales — parcialmente resueltas el 13 jul 2026.** Con la extensión de Claude in Chrome ya conectada, se intentó redimensionar la ventana a 320/375/390/768/1024/1440/2048px (`resize_window`) y también vía zoom del navegador — ninguno de los dos métodos cambió el `window.innerWidth` real (quedó fijo en 1280px), aparente limitación del entorno de esta sesión. En su lugar, se hizo una verificación en vivo real al ancho fijo disponible (1280px, dentro del rango 900–1400px): se midió `getBoundingClientRect()`/`getComputedStyle()` del `.card` en `/`, `/aprender/`, `/contacto/`, `/blog/`, `/infografia/`, `/diversifica/` y `/servicios/` — las 7 dieron exactamente 1200px de ancho, confirmando la unificación real en navegador (no solo por código fuente). Se confirmó además, con captura de pantalla, que el botón "Hablemos" de `/contacto/` no se parte en dos líneas y que `/infografia/` muestra el logo una sola vez. **Sigue pendiente:** verificación visual real en los anchos por debajo de 900px y por encima de 1400px (mobile puro y pantallas grandes) — no se pudo forzar el viewport a esos valores en esta sesión; repetir cuando el entorno permita un resize real o probar desde un dispositivo físico.
...
0d. **Confirmar en la cuenta real de Google Analytics** que los 21 eventos configurados en código (ver 1.11d) están llegando y clasificándose correctamente — solo se verificó que el código los dispara, no la recepción del lado de Google.
